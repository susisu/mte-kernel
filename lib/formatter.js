  ...
  if (delimiterRow !== undefined) {
    ...
    newRows.push(new TableRow(
      _extendArray([], tableWidth, () => new TableCell(
        _delimiterText(options.minDelimiterWidth, Alignment.DEFAULT)
      )),
      "",
      ""
    ));
  ...
  if (delimiterRow !== undefined) {
    rows.push(new TableRow(
      delimiterRow.getCells().map(cell =>
        new TableCell(_delimiterText(options.minDelimiterWidth, cell.getAlignment()))
      ),
      marginLeft,
      ""
    ));
...
export function alterAlignment(table, columnIndex, alignment, options) {
  ...
  const delimiterCells = delimiterRow.getCells();
  delimiterCells[columnIndex] = new TableCell(_delimiterText(options.minDelimiterWidth, alignment));
  const rows = table.getRows();
