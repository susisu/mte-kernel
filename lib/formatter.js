 ...
 * | ---------------- | ---------------------------------- |
 * | width            | {@link number}                     |
 * | defaultAlignment | {@link Alignment}                  |
 * | normalize        | {@link boolean}                    |
 * | wideChars        | {@link Set}&lt;{@link string} &gt; |
 * | narrowChars      | {@link Set}&lt;{@link string} &gt; |
 * | ambiguousAsWide  | {@link boolean}                    |
 ...
 * @returns {string}
 * @throws {Error} Unknown alignment.
 * @throws {Error} Default alignment is wrongly specified.
 */
export function _align(text, alignment, options) {
  const widthOptions = {
    normalize      : options.normalize,
    wideChars      : options.wideChars,
    narrowChars    : options.narrowChars,
    ambiguousAsWide: options.ambiguousAsWide
  };
  const space = options.width - _computeWidth(text, widthOptions);
  if (space < 0) {
    return text;
  }
  const computedAlignment = alignment === Alignment.DEFAULT ? options.defaultAlignment : alignment;
  switch (computedAlignment) {
  case Alignment.DEFAULT:
    throw new Error("Default alignment is wrongly specified");
  case Alignment.LEFT:
