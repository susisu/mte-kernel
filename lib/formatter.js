...
/**
 * Moves a column at the index by the specified offset.
 *
 * @param {Table} table - A completed non-empty table.
 * @param {number} columnIndex - An index of the column to be moved.
 * @param {number} offset - An offset the column is moved by.
 * @returns {Table} A new table object.
 */
export function moveColumn(table, columnIndex, offset) {
  const dest = Math.min(Math.max(columnIndex + offset, 0), table.getHeaderWidth() - 1);
  const rows = table.getRows();
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const cells = row.getCells();
