  ...
  if (delimiterRow !== undefined) {
    ...
    newRows.push(new TableRow(
      _extendArray([], tableWidth, () => new TableCell(
        _delimiterText(Alignment.DEFAULT, options.minDelimiterWidth)
      )),
      "",
      ""
    ));
 ...
 * If there is no delimiter row or the column index is out of range, returns the original table.
 */
export function alterAlignment(table, columnIndex, alignment, options) {
  const delimiterRow = table.getDelimiterRow();
  if (delimiterRow === undefined || columnIndex < 0 || delimiterRow.getWidth() <= columnIndex) {
    return table;
  }
  const delimiterCells = delimiterRow.getCells();
 ...
 *
 * @param {Table} table - A (non-empty) table object.
 * @param {number} rowIndex - An row index at which a new row will be inserted.
 * @param {TableRow} row - A table row to be inserted.
 * @returns {Table} - A new table obejct.
 */
export function insertRow(table, rowIndex, row) {
  const hasDelimiterRow = table.getDelimiterRow() !== undefined;
  const rows = table.getRows();
  rows.splice(Math.max(hasDelimiterRow ? 2 : 1, rowIndex), 0, row);
  return new Table(rows);
}
...
/**
 * Moves a row at the index by the specified offset.
 *
 * @param {Table} table - A table object.
 * @param {number} rowIndex - An index of the row to be moved.
 * @param {number} offset - An offset the row will be moved by.
 * @returns {Table} A new table object.
 */
export function moveRow(table, rowIndex, offset) {
  const hasDelimiterRow = table.getDelimiterRow() !== undefined;
  if (rowIndex === 0 || (hasDelimiterRow && rowIndex === 1)) {
    return table;
  }
  const rows = table.getRows();
  const row = rows[rowIndex];
  const dest = Math.min(Math.max(rowIndex + offset, 2), table.getHeight() - 1);
  rows.splice(rowIndex, 1);
...
export function insertColumn(table, columnIndex, column, options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (hasDelimiterRow) {
      cell = i === 1
        ? new TableCell(_delimiterText(Alignment.DEFAULT, options.minDelimiterWidth))
        : column[i > 1 ? i - 1 : i];
    }
    else {
      cell = column[i];
    ...
    if (columnIndex > cells.length) {
      // complete cells
      const num = columnIndex - cells.length;
      const comp = hasDelimiterRow && i === 1 ? cell : new TableCell("");
      for (let i = 0; i < num; i++) {
        cells.push(comp);
      }
    }
    cells.splice(columnIndex, 0, cell !== undefined ? cell : new TableCell(""));
    newRows[i] = new TableRow(cells, row.marginLeft, row.marginRight);
  }
...
export function deleteColumn(table, columnIndex, options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (columnIndex === 0 && cells.length === 1) {
      cells = [
        hasDelimiterRow && i === 1
        ? new TableCell(_delimiterText(Alignment.DEFAULT, options.minDelimiterWidth))
        : new TableCell("")
