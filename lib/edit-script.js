 ...
 * @returns {undefined}
 */
export function _applyCommand(textEditor, command) {
  if (command instanceof Insert) {
    textEditor.insertLine(command.row, command.line);
  }
  else if (command instanceof Delete) {
    textEditor.deleteLine(command.row);
  }
  else {
 ...
 * @returns {undefined}
 */
export function applyEditScript(textEditor, script) {
  for (const command of script) {
    _applyCommand(textEditor, command);
  }
}
