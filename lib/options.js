 ...
 */
export function _read(obj, key, defaultVal) {
 ...
 */
export function _readMulti(obj, keys) {
  const res = {};
  for (const [key, defaultVal] of Object.entries(keys)) {
    res[key] = _read(obj, key, defaultVal);
  }
  return res;
}
 ...
 * | property name     | type                              | description                                         | default value |
 * | ----------------- | --------------------------------- | --------------------------------------------------- | ------------- |
 * | `normalize`       | {@link boolean}                   | Normalize texts before computing text widths.       | `true`        |
 * | `wideChars`       | {@link Set}&lt;{@link string}&gt; | Set of characters that should be treated as wide.   | `new Set()`   |
 * | `narrowChars`     | {@link Set}&lt;{@link string}&gt; | Set of characters that should be treated as narrow. | `new Set()`   |
 * | `ambiguousAsWide` | {@link boolean}                   | Treat East Asian Ambiguous characters as wide.      | `false`       |
 *
 * @returns {Object} - An object that contains complete options.
 */
export function options(obj) {
  const res = _readMulti(obj, {
    formatType       : FormatType.NORMAL,
    minDelimiterWidth: 3,
    defaultAlignment : DefaultAlignment.LEFT,
    headerAlignment  : HeaderAlignment.FOLLOW
  });
  res.textWidthOptions = _readMulti(obj.textWidthOptions, {
    normalize      : true,
    wideChars      : new Set(),
    narrowChars    : new Set(),
