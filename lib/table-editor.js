...
export class TableEditor {
  ...
  moveRow(offset, options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      let altered = completed.table;
      if (newFocus.row > 1) {
        altered = moveRow(altered, newFocus.row, offset);
        newFocus = newFocus.setRow(
          Math.min(Math.max(newFocus.row + offset, 2), altered.getHeight() - 1)
        );
      }
      // format
      const formatted = formatTable(altered, options);
  ...
  deleteColumn(options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      else if (newFocus.column > completed.table.getHeaderWidth() - 1) {
        newFocus = newFocus.setColumn(completed.table.getHeaderWidth() - 1);
      }
      // delete a column
      const altered = deleteColumn(completed.table, newFocus.column, options);
      if (newFocus.column > altered.getHeaderWidth() - 1) {
  ...
  moveColumn(offset, options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      if (0 <= newFocus.column && newFocus.column <= altered.getHeaderWidth() - 1) {
        altered = moveColumn(altered, newFocus.column, offset);
        newFocus = newFocus.setColumn(
          Math.min(Math.max(newFocus.row + offset, 0), altered.getHeaderWidth() - 1)
        );
      }
      // format
      const formatted = formatTable(altered, options);
