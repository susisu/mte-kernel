...
export class TableEditor {
  ...
  moveFocus(rowOffset, columnOffset, options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      const moved = !focus.posEquals(newFocus);
      // format
      const formatted = formatTable(completed.table, options);
      // compute focus offset
      if (moved) {
    ...
    this._withTable(({ range, lines, table, focus }) => {
      ...
      if (options.smartCursor) {
        if (!this._scActive) {
          ...
          if (focus.column < 0 || formatted.table.getHeaderWidth() - 1 < focus.column) {
            this._scStartFocus = new Focus(focus.row, 0, 0);
          }
          else {
            this._scStartFocus = focus;
          }
        }
