 ...
 * @param {Object} completed - Information of the completed table.
 ...
 * @returns {number}
 */
export function _computeNewOffset(focus, completed, formatted, moved) {
  if (moved) {
    ...
    const completedFocusedCell = completed.table.getFocusedCell(focus);
    const formattedFocusedCell = formatted.table.getFocusedCell(focus);
    if (completedFocusedCell !== undefined && formattedFocusedCell !== undefined) {
      const contentOffset = Math.min(
        completedFocusedCell.computeContentOffset(focus.offset),
...
export class TableEditor {
  ...
  format(options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      newFocus = newFocus.setOffset(_computeNewOffset(newFocus, completed, formatted, false));
      // apply
  ...
  moveFocus(rowOffset, columnOffset, options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      // format
      const formatted = formatTable(completed.table, options);
      newFocus = newFocus.setOffset(_computeNewOffset(newFocus, completed, formatted, moved));
      // apply
      this._textEditor.transact(() => {
        this._updateLines(range.start.row, range.end.row + 1, formatted.table.toLines(), lines);
        if (moved) {
  ...
  nextCell(options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      // format
      const formatted = formatTable(altered, options);
      newFocus = newFocus.setOffset(_computeNewOffset(newFocus, completed, formatted, true));
      // apply
  ...
  deleteRow(options) {
    this._withTable(({ range, lines, table, focus }) => {
      ...
      // delete a row
      const altered = deleteRow(completed.table, newFocus.row);
      if (newFocus.row > altered.getHeight() - 1) {
