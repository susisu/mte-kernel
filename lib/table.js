...
export class Table {
   ...
   * *NOTE: This is not consistent for an empty table.*
   *
   ...
   * `undefined` if the focus is out of the table.
   */
  computePosition(focus, rowOffset) {
   ...
   */
  computeSelectionRange(focus, rowOffset) {
    const row = this._rows[focus.row];
    if (row === undefined) {
      return undefined;
    }
